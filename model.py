...
class OutputPredictor(nn.Module):
    def __init__(self):
        ...
        self.fc4 = nn.Linear(128, 40)  # just real concat with imag
    ...
    def forward(self, x2d, x1d_real, x1d_imag):
        ...
        x2d = F.sigmoid(self.conv1(x2d))
        x2d = F.sigmoid(self.conv2(x2d))
        x2d = x2d.view(x2d.size(0), -1)  # Flatten
        x2d = F.sigmoid(self.fc1_2d(x2d))

        # Process the 1D array
        x1d_real = F.sigmoid(self.fc1_1d_real(x1d_real))
        x1d_imag = F.sigmoid(self.fc1_1d_imag(x1d_imag))
